...
class Blockchain {
  ...
  /**
   * The _getCurrentTimeStamp allow us get the date format
   * @returns Date
   */
  ...
  getStarsByWalletAddress(address) {
    ...
    return new Promise((resolve, reject) => {
      self.chain.filter((block) => {
        block.getBData().owner == address ? stars.push(block) : null;
      });
      resolve(stars);
    });
  }
   ...
   * (?) how validate the previousBlockHash if the hash that was create,
   *  was before to create the hash
   * (?)
   */
  validateChain() {
    let self = this;
